...
(() => {
  ...
  // Required DOM IDs (actual IDs, not object keys)
  const REQUIRED_IDS = [
  ...
  for (const id of REQUIRED_IDS) {
    if (!$(id)) {
      console.error("[chavruta] missing element id:", id);
      alert(`Chavruta wiring error: missing #${id}. Check /chavruta/index.html IDs.`);
      return;
    }
  }
  ...
  function modePromptHint(mode) {
    ...
    if (mode === "chavruta") return "Peshat → one classical note → 2–6 questions back.";
